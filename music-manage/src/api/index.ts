...
const HttpManager = {
    ...
    getAllSong: () => get(`song`),
    // 分页获取歌曲
    getSongPages: ({ currentPage, pageSize, name }) => 
      get(`song/page?pageNum=${currentPage}&pageSize=${pageSize}&name=${name || ''}`),
    // 返回指定歌手ID的歌曲
    ...
    //近 30 日用户活跃趋势
    getRecentUserActivity:() => get(`info/user/activity`),

    //新增内容趋势 (周/月)
    getNewContentTrend:()=> get(`info/content/trend`),
