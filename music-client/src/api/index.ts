...
const HttpManager = {
  ...
  //attachImageUrl: (url) => url ? `${getBaseURL()}/${url}` : "https://cube.elemecdn.com/e/fd/0fc7d20532fdaf769a25683617711png.png",
    ...
    attachImageUrl: (url) => {
      if (!url) {
        return HttpManager.getRandomImageUrl();
      }
      
      if (url.startsWith('http')) {
        return url;
      } else {
        return url ? `${getBaseURL()}${url}` : HttpManager.getRandomImageUrl();
      }
    },
    ...
    getRandomImageUrl: () => {
      const services = [
        `https://t.alcy.cc/mp/`,
        `https://img.paulzzh.com/touhou/`,
        `https://picsum.photos/300/300?random`,
        `https://api.btstu.cn/sjbz/api.php`,
        `https://moe.jitsu.top/api`,//危险头像
       `https://source.unsplash.com/random/300x300?${Date.now()}`,
        `https://dummyimage.com/300x300/cccccc/969696?text=Music&font=lobster`,
  ...
  // 获取最热歌单
  getHotSongLists: () => get("/songList/hot?limit=10"),
  ...
  getSingerOfSex: (sex) => get(`singer/sex/detail?sex=${sex}`),
    ...
    getHotSingers: () => get("/singer/hot?limit=10"),
//========================================================================================================================================================================================
  ...
  //获取的推荐歌曲
  getRecommendSongList: () => get("recommend/data"),
