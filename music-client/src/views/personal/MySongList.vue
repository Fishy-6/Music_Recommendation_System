            ...
            <el-dropdown>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item
                    :icon="Delete"
                    @click.stop="removeSongFromList(scope.row)"
                    >移除歌曲</el-dropdown-item>
...
import { useRoute } from "vue-router";
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const songListId = ref(route.params.id || route.query.id);
      console.log("当前歌单ID:", songListId.value);
  ...
  // 控制弹窗的变量
      ...
      async function removeSongFromList(row) {
        if (!checkStatus()) return;

        // 从列表中删除歌曲
        const result = (await HttpManager.deleteListSongOfId(row.id,songListId.value )) as ResponseBody; // 示例方法名，请替换为实际接口调用
